...
## 15. Additive status — 2026-07-19 08:10 UTC

Append-only. Closes the "first scheduled full-run evidence remains pending" note in section 14. It changes no enforcement boundary.

The first natural full model-health scan under `4007b3f` ran on the existing timer at 08:05:05 UTC (`--mode=full`, exit 0) and was observed passively. It wrote the first `model-catalog.json` (schema v1, `bounded-provider-inventory-v1`, runId `9e7c4396-...`) with all 11 provider catalogs `complete=true` and none carried forward. Discovery stayed observation-only: routing-config SHA-256 hashes (`0c9f7764...e093` / `922a5914...a79`) and LiteLLM identity (PID `2137446`, `NRestarts=0`) were unchanged, and no route was activated. AIHubMix remained inert — credential `valid` (200) but `gatesModels=0`. Live credential separation showed 9 `valid` / 2 `rate_limited` (NVIDIA, OpenCode Zen at 429), with GitHub recovered and Cloudflare/Groq resolved to `valid`. Full sanitized evidence is in SPEC 49 section 12 and the 2026-07-19 Vault entry. The R1/R2/enforcement gates in sections 10-13 are unaffected and remain pending.
