...
## 12. First natural full-scan evidence — 2026-07-19 08:05 UTC

The first natural full run under `4007b3f` fired on the existing `model-health-check.timer` (`--mode=full`) at 08:05:05 UTC and completed exit 0. It was observed passively: no manual scan, no config apply, no hot-add, and no LiteLLM restart.

- `model-catalog.json` was produced root-owned `0600`: schema v1, `policyVersion=bounded-provider-inventory-v1`, `runId=9e7c4396-fc1d-4268-afaf-35db20055fc0`. All 11 provider adapters reported `complete=true` and `carriedForward=false` — no collapse, oversize, or carry-forward erasure. Sanitized raw/eligible counts: aihubmix 356/27, openrouter 334/23, github 37/35, groq 15/10, zen 55/7, cloudflare 26/(eligible predicate reported 32 — a count discrepancy to reconcile), cerebras 3/3, nvidia 119/112, gemini 54/36, alibaba 51/10, opencode 15/8. Drift vs the 2026-07-18 observation: OpenRouter 344->334; Cloudflare removed 6.
- Observation-only held: `/etc/litellm/config.yaml` stayed SHA-256 `0c9f7764...e093` and `/etc/tib-builder/gateway.yaml` stayed `922a5914...a79`; LiteLLM stayed PID `2137446`, `NRestarts=0`, active since 2026-07-18 14:03:13 UTC.
- AIHubMix admitted nothing. `credential-health.json` reports `AIHUBMIX_API_KEY` `valid` (HTTP 200) with `gatesModels=0`; the key authenticates for listing/balance while zero logical routes are wired to it. Section 10's "zero durably verified working AIHubMix free models" decision holds under a live scan.
- Credential separation, live: 9 `valid`, 2 `rate_limited` (`NVIDIA_NIM` and `OPENCODE_ZEN`, both HTTP 429). `GITHUB_TOKEN` recovered `rate_limited->valid`; Cloudflare and Groq resolved the prior conservative `unknown` to `valid` after the bounded official-envelope fix, exactly as SPEC 48 predicted.
- Minor follow-up: `model-discovery-log.jsonl` (0644) did not surface per-model admission-decision fields to a whitelisted sanitized reader. Confirm the log records explicit admission/redemption decisions per model; this is observability only and does not affect the activation proof above.

No activation proposal was promoted, ranked, or added to any fallback chain. Any future admission still requires the separate identity-revalidating apply path.
