...
## 11. Slice 0 implementation status — 2026-07-19 UTC

Slice 0 (contain raw OpenCode exposure) is implemented and accepted against its external + loopback gate. The raw upstream is now bound to loopback, its public raw reverse-proxy routes are closed (the authenticated Control Surface proxy path is unchanged), its provider secrets were moved to a root-owned 0600 EnvironmentFile, and a defense-in-depth firewall deny was added. An independent read-only adversarial review (Claude Sonnet 5) found no unauthenticated-access gap. Credential rotation of the previously-exposed keys and removal of the dashboard-managed public hostname/DNS are operator follow-ups; exact coordinates and the rotation runbook are held only in the private AI Vault. This unblocks Slice 1 (durable ownership-aware registry and the server-side hidden-session visibility invariant), which must still pass its own external and loopback acceptance before any "hidden forever" claim.

## 12. Slice 1 implementation status — 2026-07-19 UTC

Slice 1 is implemented, deployed, and accepted. Dashboard schema v12 adds the durable session, run, event, visibility-receipt, artifact, and fenced-lease records; the database, WAL/SHM files, and legacy harness state files are root-owned `0600`. A one-time exact manifest records 2,490 immutable legacy internal-session receipts and preserves 77 operator/development OpenCode sessions. Future probes use only the exact reserved marker; no broad runtime title or path heuristic remains.

The former generic OpenCode passthrough is replaced by a method-and-path allowlist with one tenant/owner/ACL/role predicate across list, count, direct-id, mutation, and filtered SSE paths. Unknown and hidden ids fail before the upstream request. OpenCode, Codex, Claude, and Gemini state is registered durably; writer launch paths acquire inode/realpath-fenced leases; runs and ordered digest-backed events reconstruct after restart; normal home/discovery counts are request-scoped. The root-only diagnostic requires explicit internal access and records an audit row.

Acceptance evidence: `bun test --timeout 30000` passed 1,340/1,340 across 167 files; `bun run check` passed; the isolated authenticated smoke returned 401 for anonymous registry access, 404 for hidden direct ids and unknown proxy routes, and only the 77 preserved OpenCode sessions; production reports schema 12, 2,490 receipts, 81 visible registered sessions (77 OpenCode plus 4 Codex), and matching OpenCode counts on the governed list, home, and discovery surfaces. The raw upstream remains loopback-only. Slice 2, real multi-session terminal, is the next implementation slice; no Slice 2–6 parity claim is made here.
