...
## Status Note - 2026-07-10 07:50 UTC - ULTRAPLAN A4 Gateway Key Rotation

Scheduler-plan implementation remains complete; active work continued in `/root/control-surface-plans/ULTRAPLAN.md` Phase 3 A4. Codex completed `rotate:gateway-key:<id>` with schema-backed grace-period dual-validity, `/api/gateway/keys/:id/rotate`, global executor/catalog wiring, and focused key/executor/catalog tests. Evidence: focused tests 19/0, 17/0, and 5/0; `bun run typecheck`; `bun run build`; `bun run check`; `git diff --check`; isolated `:3299` `/health` plus `/api/actions/catalog` smoke with `gatewayKeys: ok`. Next active ULTRAPLAN item: A4 route override with TTL.

## Status Note - 2026-07-10 14:09 UTC - Persistent Root Terminal

Added and deployed `/terminal` as a core Control Surface route. The backend uses Bun's native PTY API over an operator-authenticated, same-origin WebSocket and attaches each client to persistent root tmux session `tib-root`, so CLI work survives phone sleep, browser disconnects, device switching, and Control Surface restarts. The UI uses xterm.js with terminal resize synchronization, automatic reconnect, search/clipboard controls, AI CLI command insertion, focus mode, and a phone soft-key rail with 44px touch targets. Security boundaries include bootstrap-owner-only auth, Origin validation, connection cap/rate limiting, fixed root/tmux command construction, and lifecycle-only audit records; command input/output is not logged. Evidence: `bun run check`, full DB/API/terminal tests, focused terminal tests 5/0, diff check, ephemeral persistence smoke, live localhost and public Cloudflare/Caddy root-WebSocket smokes, and live desktop/tablet/iPhone visual checks 3/3 with no console/page/request failures. `control-surface.service` is active; public `/terminal` returns 200 and unauthenticated terminal status returns 401.

## Status Note - 2026-07-18 14:43 UTC - Protected APIs, Model Health, and Routing Repair

- Landed and deployed the Control Surface repair slice through `2fe5267`: protected the remaining home/showcase APIs and SSE (`a009091`, `03cb484`), removed the phantom Vast fresh-host leak (`800d8c8`), shipped seven-state model health and evidence-grouped `/models` (`69e3535`, `8ddcc9f`, `1bbc176`), and fixed broad bare-digit server-error classification (`2fe5267`).
- Evidence: `bun run check`; bounded API suite 546 pass / 0 fail; focused classifier/router suite 24 pass / 0 fail; isolated API-only fresh-host gate 145 HONEST / 0 LEAK / 0 CRASH / 0 5xx; live `/health` ok; deployed build hash `2fe5267`; protected-route auth and authenticated SSE smokes passed.
- Live model snapshot before the classifier-only deploy: 193 routes, 0 rows missing health fields, with 14 healthy, 39 unhealthy, and 140 unknown. No Playwright or multi-viewport browser run was performed on this VPS.
- Companion routing commits `/opt/mimoun@d117b84` and `ae0959c` make reprobe the sole fallback writer and add conservative ledger reconciliation in shadow mode. The shadow report proposes exactly one prune, `zen-deepseek-v4-flash-free` (0/45 trusted seven-day calls across 124.9 hours), but `enforced=false`; dry-run hashes proved LiteLLM config, gateway config, and reprobe state unchanged.
- R1 remains observational until three stable scheduled reprobe cycles and the 48-hour restart target are proven. R2 enforcement is blocked until a durable tombstone, transient-ledger behavior, and explicit recovery rule are specified.
- `SPEC_45_CLASSIFY_ERROR_AND_R6_ACCEPTANCE.md` supersedes the colliding/stale SPEC 44 and defines strict no-Playwright acceptance, read-only verification, immutable evidence, and real-work gates.
- Next: collect three scheduled R1/R2 shadow observations; design the durable R2 apply/recovery slice; implement the remaining SPEC 45 verification tooling and allowed UI unit seam; obtain explicit authorization for the real editorial and builder acceptance runs.

## Status Note - 2026-07-19 12:35 UTC - Know Integration and Agent Workspace Slice 1

- Added and deployed first-class authenticated `/know` navigation, health/ops/doctor visibility, and governed fixed refresh/build actions. The daily Know push streak remains observe-only from Control Surface.
- Completed Agent Workspace Slice 1: schema v12 durable registry/run/event/visibility/artifact/lease storage, immutable exact legacy visibility receipts, ownership/ACL/role enforcement, deny-by-default OpenCode proxying, filtered SSE, durable legacy harness registration, and fenced writer leases.
- Production acceptance reports 2,490 immutable internal receipts, 77 preserved visible OpenCode sessions, 4 visible Codex sessions, matching governed/home/discovery counts, hidden direct ids and unknown routes at 404, anonymous registry at 401, and raw OpenCode still loopback-only. All DB/WAL/SHM and harness state files are root-owned `0600`.
- Know now has a reusable 13-scenario professional HTML/text email system and explicit default-off story-email preference. Story mail accepts only the signed-in account, an opted-in preference, a live story, and the bounded rate limit; no unsolicited email was sent.
- Evidence: Control Surface full suite 1,340 pass / 0 fail; `bun run check`; isolated and production smokes; Know `npm run check`, typecheck, build, and local/public health 200 with schema 4 and 8 live stories.
- Known honest degradation: Know doctor remains degraded only because no configured Know logical authoring model is currently available. Next Agent Workspace item is Slice 2; rotate the chat-exposed Microsoft 365 application secret separately in Entra.
